...
int getgatewayandiface(char *gateway_address, char *interface)
{
    ...
    memset(gateway_address, 0, INET_ADDRSTRLEN);
    ...
    for ( ; NLMSG_OK(nlh, received_bytes); nlh = NLMSG_NEXT(nlh, received_bytes))
    {
        ...
        for ( ; RTA_OK(route_attribute, route_attribute_len); route_attribute = RTA_NEXT(route_attribute, route_attribute_len))
        {
            ...
            case RTA_OIF:
                ...
                break;
            case RTA_GATEWAY:
                inet_ntop(AF_INET, RTA_DATA(route_attribute), gateway_address, INET_ADDRSTRLEN);
                break;
            default:
                break;
            }
        ...
        // if ((*gateway_address) && (*interface)) {
            // fprintf(stdout, "Gateway_fn %s for interface %s\n", gateway_address, interface);
            // break;
        // }
    }
